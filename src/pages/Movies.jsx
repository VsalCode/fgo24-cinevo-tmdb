...
const Movies = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // const searchMovies = await axios.get(`https://api.themoviedb.org/3/search/movie?${searchParams}`);
  ...
  //   const filteredSearch = movies.filter((item) => item.genre_ids.some((genre) => genre.name === e) )
          ...
          <div className="flex flex-col lg:flex-row gap-6 md:gap-8 mb-8 md:mb-12">
            <form onSubmit={handleSubmit(handleSearch)} className="w-full lg:w-1/2">
              ...
              <span className="bg-white border-third text-primary flex items-center border rounded-full w-full max-w-[350px] px-5 py-2">
                ...
                <input type="text" className="outline-none border-0 ps-3 w-full grow text-secondary" placeholder="Search Movie...." {...register('query')} />
              </span>
            </form>
            ...
            {currentMovies.map((item) => (
              <Link to={`/movieDetail/${item.id}`} key={item.id} className="flex flex-col justify-between w-full max-w-xs transition-transform duration-300">
                <div className="relative">
                  ...
                  <img className="rounded-xl object-cover w-full h-80 md:h-96 hover:scale-105" src={`https://image.tmdb.org/t/p/w500${item.poster_path}`} alt={item.title} />
                </div>
                <div className="flex flex-col pt-4 gap-3 text-center">
                  ...
                  <Link to={`/movieDetail/${item.id}`} className="bg-third text-white text-sm md:text-base font-bold py-2 px-4 rounded-md hover:bg-secondary transition-colors">
                    View Details
                  </Link>
                </div>
              </Link>
